...
public class RIVTester {
    ...
    private func processTests() -> String {
        ...
        for (index, test) in tests.enumerate() {
            result += testOutput(test, number: index + 1, succeeded: &succeeded, pending: &pending)
        }
        
        return result + endLine(totalCount: self.tests.count, succeeded: succeeded, pending: pending)
    }
    
    private func endLine(totalCount totalCount: Int, succeeded: Int, pending: Int) -> String {
        return "\nExecuted \(tests.count) test(s) | \(succeeded) succeeded | \(tests.count - succeeded - pending) failed | \(pending) pending"
    }
    
    private func testOutput(test: RIVTest, number: Int, inout succeeded : Int , inout pending : Int) -> String {
